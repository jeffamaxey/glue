...
STATE_CLASS = {
    'ImageLayerArtist': ImageLayerState,
    'ScatterLayerArtist': ScatterLayerState,
    'SubsetImageLayerArtist': ImageSubsetLayerState,
}
        ...
        if layer['_type'].split('.')[-1] == 'RGBImageLayerArtist':
            for icolor, color in enumerate('rgb'):
                new_layer = {
                    ...
                    'norm': layer[f'{color}norm'],
                    'zorder': layer['zorder'],
                    'visible': layer['color_visible'][icolor],
                    'color': color,
                }
